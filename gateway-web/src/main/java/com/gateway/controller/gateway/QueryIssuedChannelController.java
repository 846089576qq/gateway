...
import com.gateway.payment.service.adapter.IQueryIssuedChannelServiceAdapter;
 ...
 * 代付通道查询入口
 * 
 * 作者：王政 创建时间：2017年3月6日 下午5:55:40
 */
@Controller
@RequestMapping("/issuedChannel")
public class QueryIssuedChannelController extends BaseController {

	private static final Logger logger = LoggerFactory.getLogger(QueryIssuedChannelController.class);

	@Reference
	IQueryIssuedChannelServiceAdapter queryChannelServiceAdapter;
	...
	public String jsonPayment(HttpServletRequest request, HttpEntity<String> entity) {
		...
		try {
			...
			try {
				...
				try {
					if (!"1".equals(paramMap.get(BaseParamConstant.param_is_zitopay_v1))) {
						publicKey = queryChannelServiceAdapter.getMerchantPublicKey(pid, rd.getSerialNo());// 根据应用ID查询商户公钥
					}
				} catch (Exception e) {
			 ...
			 * 2.参数验证
			 */
			Map<String, Object> checkMap = queryChannelServiceAdapter.checkQueryChannelParameter(paramMap, rd.getSerialNo());
			...
			Map<String, Object> permissionMap = queryChannelServiceAdapter.checkQueryChannelPermission(merchantId, gatewayId, rd.getSerialNo());
			...
			Map<String, Object> queryChannelResultMap = queryChannelServiceAdapter.execChannelQuery(zitopayPerson, zitopayGeteway, zitopayPersonGeteway, paramMap, rd.getSerialNo());
			logger.warn("融智付-json通道查询调用,报文序列号:{},调用结果:{}", rd.getSerialNo(), queryChannelResultMap);
			rsd.setResponseTips(ResponseInfoEnum.valueOf(queryChannelResultMap));
			if (!ResponseInfoEnum.调用成功.equals(queryChannelResultMap)) {// 判断支付结果并返回
				return rsd.toEncryptString(publicKey);
			}
			rsd.setData(queryChannelResultMap);
			logger.info("融智付-json,通道查询响应报文:{}", rsd);
			return rsd.toEncryptString(publicKey);
