...
import com.gateway.payment.service.adapter.ITransferServiceAdapter;
...
public class TransferController extends BaseController {
	...
	@Reference
	ITransferServiceAdapter transferServiceAdapter;
	...
	public String jsonPayment(HttpServletRequest request, HttpEntity<String> entity) {
		...
		try {
			...
			try {
				...
				try {
					if (!"1".equals(paramMap.get(BaseParamConstant.param_is_zitopay_v1))) {
						publicKey = transferServiceAdapter.getMerchantPublicKey(pid, rd.getSerialNo());// 根据应用ID查询商户公钥
					}
				} catch (Exception e) {
			 ...
			 * 2.参数验证
			 */
			Map<String, Object> checkMap = transferServiceAdapter.checkTransferParameter(paramMap, rd.getSerialNo());
			...
			Map<String, Object> permissionMap = transferServiceAdapter.checkTransferPermission(merchantId, gatewayId, rd.getSerialNo());
			...
			Map<String, Object> drawResultMap = transferServiceAdapter.execTransfer(zitopayPerson, zitopayGeteway, zitopayPersonGeteway, paramMap, rd.getSerialNo());

			logger.warn("融智付-json转账调用,报文序列号:{},调用结果:{}", rd.getSerialNo(), drawResultMap);
			rsd.setResponseTips(ResponseInfoEnum.valueOf(drawResultMap));
			if (!ResponseInfoEnum.调用成功.equals(drawResultMap)) {// 判断转账结果并返回
				return rsd.toEncryptString(publicKey);
			}
			rsd.setData(drawResultMap);
			logger.info("融智付-json,转账响应报文:{}", rsd);
			return rsd.toEncryptString(publicKey);
