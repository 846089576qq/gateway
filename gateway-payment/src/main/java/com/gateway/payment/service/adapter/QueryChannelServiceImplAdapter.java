...
public class QueryChannelServiceImplAdapter extends BaseServiceImplAdapter implements IQueryChannelServiceAdapter {
	...
	public Map<String, Object> checkQueryChannelParameter(Map<String, String> paramMap, String serialNo) throws ServiceException {
		try {
			...
			String merchantId = paramMap.get(PaymentParamConstant.param_merchantId);// 商户id，由融智付分配
			if (StringUtils.isEmpty(merchantId)) {
				return ResponseInfoEnum.参数为空.getMap("参数[" + PaymentParamConstant.param_merchantId + "]不能为空");
			}

			Integer gatewayId = Integer.valueOf(paramMap.get(QueryChannelParamConstant.param_gatewayId)); // 通道ID，由融智付分配
			if (null == gatewayId) {
				return ResponseInfoEnum.参数为空.getMap("参数[" + QueryChannelParamConstant.param_gatewayId + "]不能为空");
			}

			String orderId = paramMap.get(QueryChannelParamConstant.param_order_id); // 融智付订单号
			if (StringUtils.isBlank(orderId)) {
				return ResponseInfoEnum.参数为空.getMap("参数[" + QueryChannelParamConstant.param_order_id + "]不能为空");
			}
	...
	public Map<String, Object> execChannelQuery(PersonEntity zitopayPerson, GatewayEntity zitopayGeteway, PersonGatewayEntity zitopayPersonGeteway, Map<String, String> paramMap, String serialNo) throws ServiceException {
		try {
			...
			return executeQuery(orderEntity, serialNo);// 处理查询、通知商户、分润等;
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
